...
$c \cdot P^2 \cdot \left(\frac{N}{P}\right)^3 \cdot t_\text{flop} = c \cdot \frac{N^3}{P} \cdot t_\text{flop}$

since we have to carry out $P^2$ multiplications of $\frac{N}{P} \times \frac{N}{P}$ matrices. The cost of tranferring data (size $\frac{N^2}{P}$) across cores (happens $P-1$ times) is:

$(P-1) \cdot \frac{N^2}{P} \cdot t_\text{comm} \approx N^2 \cdot t_\text{comm}$

So, the total computation time is:

$T(P,N) = c \cdot \frac{N^3}{P} \cdot t_\text{flop} + N^2 \cdot t_\text{comm} \approx c \cdot \frac{N^3}{P} \cdot t_\text{flop}$

where we keep the leading term ($N^3$) when the matrix size becomes sufficiently large. As a result, we expect the computing time to be proportional to $\frac{N^3}{P}$. If we plot $T(P,N)$ vs. $\frac{N^3}{P}$ we get the following graph (both axes are in log scale):

<p align="center"> 
  <img src="pics/curves.png" width="500" />
</p>

The relationship is linear, as expected by the complexity analysis.
